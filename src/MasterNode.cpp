...
MasterNode::MasterNode(dss::PetriNet *petri,
    ...
    LOG_INFO(get_logger(), "Modules count: %d", m_petri->getModulesCount());
    ...
    for (size_t i{1}; i < m_petri->getModulesCount(); ++i) {
        ...
        LOG_INFO(get_logger(), "Created client for firing info: %s", client_name.c_str());
...
auto MasterNode::run() -> void {
    switch (m_current_state) {
        case state_t::GET_SYNC_FUSION:
            LOG_INFO(get_logger(), "Current SM: GET_SYNC_FUSION");
            m_current_state = state_t::INIT;
            break;

        case state_t::INIT:
            LOG_INFO(get_logger(), "Current SM: INIT");
        ...
        case state_t::BUILD_INITIAL_META_STATE:
            LOG_INFO(get_logger(), "Current SM: BUILD_INITIAL_META_STATE");
            buildInitialMetaState();
            m_current_state = state_t::BUILD_META_STATE;
            break;

        case state_t::BUILD_META_STATE:
            LOG_INFO(get_logger(), "Current SM: BUILD_META_STATE");
            if (m_ack_modules.all()) {
                m_current_meta_state->setName(m_metastate_building_name);
                LOG_INFO(get_logger(), "Built Metastate: %s", m_current_meta_state->toString().c_str());
        ...
        case state_t::POP_METASTATE:
            LOG_INFO(get_logger(), "POP_METASTATE: Stack size: %ld", m_meta_states_stack.size());
            if (m_meta_states_stack.empty()) {
                m_current_state = state_t::TERMINATE_BUILDING;
            } else {
                m_current_meta_state = m_meta_states_stack.top();
                LOG_INFO(get_logger(), "POP_METASTATE: Popped MS: %s", m_current_meta_state->toString().c_str());
        ...
        case state_t::PROCESS_FIRE_SYNC_FINISH:
            LOG_INFO(get_logger(), "Current SM: PROCESS_FIRE_SYNC_FINISH");
            if (m_ack_modules.all()) {
                m_ack_modules.reset();
...
auto MasterNode::response_receiver(const ros2dss::Response &resp) -> void {
    LOG_INFO(get_logger(), "Received: %s", resp.msg.c_str());
    if (resp.msg == "ACK" and m_current_state == state_t::INIT) {
        ...
        // LOG_INFO(get_logger(), "Received: %s %s", resp.msg.c_str(), resp.scc.c_str());
...
auto MasterNode::fireSyncTransition() -> bool {
    if (ml_enabled_fusion_sets.empty()) {
        LOG_INFO(get_logger(), "No enabled fusion set...");
        return false;
    }
    ...
    for (uint32_t i{1}; i < m_petri->getModulesCount(); ++i) {
        ...
        if (m_petri->getManageTransitionFusionSet()->isFusionSetSyncedOnModule(transition, i)) {
            ...
            for (auto e: res) {
                LOG_INFO(get_logger(), "Received (%s,%s) ", e.source.c_str(), e.target.c_str());
    ...
    for (auto &e_tuple: l_pair_source_ms) {
        LOG_INFO(get_logger(), "Source product: %s, Destination metastate: %s",
        ...
        if (auto dest_ms{m_module_ss->findExtendedMetaState(std::get<1>(e_tuple))}; dest_ms) {
            LOG_INFO(get_logger(), "Meta state: %s is already inserted!",
                        dss::arrayModelToStdString(std::get<1>(e_tuple)).c_str());
            ...
            LOG_INFO(get_logger(), "Meta state: %s is new!", dss::arrayModelToStdString(new_ms->getName()).c_str());
...
auto MasterNode::executeFireSyncTransitionRequest(const uint32_t id_server,
    ...
    LOG_INFO(get_logger(), "Firing transition %s\n", transition.c_str());
    return res;
}
